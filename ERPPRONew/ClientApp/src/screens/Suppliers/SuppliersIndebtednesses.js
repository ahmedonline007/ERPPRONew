...
class SupplierIndebtednesses extends Component {
    ...
    componentWillReceiveProps(nextState, prevState) {
        if (nextState.listIndebtednesses && nextState.listIndebtednesses.length > 0) {
            if (this.state.customerId != "") {

                let ListPayed = nextState.listIndebtednesses.filter(x => x.customerId == this.state.customerId.value);

                this.setState({
                    isLoading: false,
                    show: false,
                    listIndebtednesses: ListPayed
                });
            } else {
                this.setState({
                    isLoading: false,
    ...
    componentDidMount() {
        ...
        this.props.actions.getAllIndebtedness(true);
    }
    ...
    handleChange(value) {
        let objRow = this.props.listIndebtednesses.filter(x => x.customerId == value.value);
    ...
    EditCreditDepit(value) {
        ...
        if (value.debt <= 0) {
            ...
            this.props.actions.AddEditIndebtednesses(value);
        }
    }
    ...
    handleDelete = () => {
        ...
        this.props.actions.DeleteIndebtedness(this.state.selected);
    ...
    handleResetSearch = () => {
        let listTransaction = this.props.listIndebtednesses.filter(x => x.customerId === this.state.customerId.value);
        ...
        this.setState({
            listIndebtednesses: listTransaction
        });
    }
            ...
            <Fragment>
                <div className="main_content_container">
                    <div className="page_content">
                        <br />
                        <br />
                        <h1 className="heading_title"> تسوية مديونية المورد بالزيادة</h1>
                        ...
                        {/* List Of Data */}
                        <ReactTable
                            data={this.state.listIndebtednesses}
                            columns={this.cells}
                ...
                <div className="row" id="DivPrint1" style={{ display: "none" }}>
                    <h1 className="heading_title ">سداد المورد </h1>
                    ...
                    <Table striped bordered hover>
                        ...
                        <tbody>
                            {this.state.listIndebtednesses.length > 0 ?
                                this.state.listIndebtednesses.map((item, i) => {
                                    return (
                                        <tr key={i}>
                                            <td>{item.debt}</td>
                                            <td>{item.debtDate}</td>
...
const mapStateToProps = (state, ownProps) => ({
    ...
    listIndebtednesses: state.reduces.listIndebtednesses
});
...
export default connect(mapStateToProps, mapDispatchToProps)(SupplierIndebtednesses);