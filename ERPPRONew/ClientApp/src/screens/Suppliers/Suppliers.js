...
class Suppliers extends Component {
    ...
    componentDidMount() {
        this.props.actions.getAllSupplierCustomer(true);
    }
    ...
    addEditDocument = (values) => {
        ...
        axios.get(`CheckExistsSupplierCustomer?name=${values.name}&flag=${true}&id=${values.id}`).then(result => {
            if (result.data === true) {
                ...
                this.props.actions.addEditSupplier(values);
            }
        });
    }
    ...
    updateCreditDepit = (values) => {
        ...
        this.props.actions.updateCreditDebit(values);
    }
    ...
    handleDelete = () => {
        ...
        this.props.actions.deleteSupplier(this.state.selected[0]);
    ...
    render() {
        ...
        let sumCredit = this.props.listSupplier.length > 0 ? this.props.listSupplier.map(x => x.credit).reduce((a, c) => { return a + c }) : 0
        let sumDebit = this.props.listSupplier.length > 0 ? this.props.listSupplier.map(x => x.debit).reduce((a, c) => { return a + c }) : 0
            ...
            <Fragment>
                <div className="main_content_container">
                    <div className="page_content">
                        <br />
                        <br />
                        <h1 className="heading_title">عرض كل الموردين</h1>
                ...
                <div id="DivPrint" style={{ display: "none" }}>
                    ...
                    <Table striped bordered hover>
                        ...
                        <tbody>
                            {
                                this.props.listSupplier.length > 0 ?

                                    this.props.listSupplier.map((item, i) => {
                                        return (<tr key={i}>
                                            <td>{item.name}</td>
                                            <td>{item.address}</td>
                                            <td>{item.phone}</td>
                                            <td className="cell" style={{ color: "red" }}> {item.credit}</td>
                                            <td className="cell" style={{ color: "red" }}> {item.debit}</td>
                                        </tr>)
                                    })
                                    : <tr></tr>
                            }
                            ...
                            {({ handleSubmit, handleChange, handleBlur, values, touched, isValid, errors, }) => (
                                <Form noValidate onSubmit={handleSubmit} style={{ fontWeight: 'bold', fontSize: '25px' }}>
                                    <Form.Group controlId="name">
                                        <Form.Label>إسم المورد</Form.Label>
                ...
                <Modal show={this.state.showSupplierInfo} onHide={this.handleClose.bind(this)} style={{ opacity: 1, marginTop: '10%' }}>
                    ...
                    <Modal.Body className="modal-header">
                        <Formik validationSchema={schemaEdit} onSubmit={(values) => { this.addEditDocument(values) }}
                            initialValues={{
                                id: this.state.objSupplier.id,
                                name: this.state.objSupplier.name,
                                address: this.state.objSupplier.address,
