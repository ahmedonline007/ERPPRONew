...
const schema = Yup.object({
    expencesName: Yup.string().required("برجاء إدخال إسم المصاريف").max(200, "الحد الاقصى للكتابة 200 حرف فقط"),
...
class Expences extends Component {
    ...
    componentWillReceiveProps(nextState, prevState) {
        if (nextState.listExpences && nextState.listExpences.length > 0) {

            this.setState({
                isLoading: false,
                show: false,
                selected: [],
                listExpences: nextState.listExpences
            });
        } else {
            this.setState({
                isLoading: false
    ...
    componentDidMount() {
        this.props.actions.getAllExpences();
    }
    ...
    addEditDocument = (values) => {
        ...
        values.expencesName = values.expencesName;
        values.cost = parseFloat(values.cost);
        values.description = values.description;
        values.date = this.state.startDate;

        this.props.actions.AddEditExpences(values);
    ...
    handleDelete = () => {
        this.props.actions.deleteExpences(this.state.selected[0]);
        ...
        return {
            onClick: (e, handleOriginal) => {
                ...
                if ((e.target.className === "Edit" || e.target.className === "rt-td") && e.target.className !== "Add") {
                    ...
                    originalSup.expencesName = rowInfo.original.expencesName;
    ...
    handleSearch = () => {
        ...
        let result = this.props.listExpences.filter(d => {
    ...
    handleResetSearch = () => {
        this.setState({
            listExpences: this.props.listExpences
        });
    }
            ...
            <Fragment>
                                ...
                                <Form noValidate onSubmit={handleSubmit} style={{ fontWeight: 'bold', fontSize: '25px' }}>
                                    <Form.Group controlId="expencesName" >
                                        ...
                                        <Form.Control type="text"
                                            ...
                                            name="expencesName"
                                            value={values.expencesName}
                                            onBlur={handleBlur}
                                            autoComplete="off"
                                            isInvalid={!!errors.expencesName}
                                        />
                ...
                <div id="DivPrint" style={{ display: "none" }}>
                    ...
                    <Table striped bordered hover>
                        ...
                        <tbody>
                            {
                                this.state.listExpences.length > 0 ?
                                    this.state.listExpences.map((item, i) => {
                                        return (<tr key={i}>
                                            <td>{item.expencesName}</td>
                                            <td>{item.cost}</td>
...
const mapStateToProps = (state, ownProps) => ({
    listExpences: state.reduces.listExpences
});
