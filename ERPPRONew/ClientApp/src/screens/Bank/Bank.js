...
const schema = Yup.object({
    name: Yup.string().required("برجاء إدخال البنك").max(100, "الحد الاقصى للكتابة 100 حرف فقط"),
    cost: Yup.number().required("برجاء قيمة المبلغ").min(0, "عفوا برجاء المبلغ أكبر من صفر")
});
...
class Bank extends Component {
    ...
    handleChange(e, feild) {
        ...
        let originalSup = this.state.objBank;
    ...
    addEditDocument = (values) => {
        ...
        this.props.actions.addEditBank(values);
    }
    ...
    handleDelete = () => {
        this.props.actions.deleteBank(this.state.selected[0]);
            ...
            <Fragment>
                <div className="main_content_container">
                    <div className="page_content">
                        <br />
                        <br />
                        <h1 className="heading_title">عرض كل البنوك</h1>
                ...
                <Modal show={this.state.show} onHide={this.handleClose.bind(this)} style={{ opacity: 1, marginTop: '10%' }}>
                    ...
                    <Modal.Body className="modal-header" style={{ marginTop: '50px' }}>
                        <Formik validationSchema={schema} onSubmit={(values) => { this.addEditDocument(values) }}
                            initialValues={{
                                id: this.state.objBank.id,
                                name: this.state.objBank.name,
                                description: this.state.objBank.description,
                                cost: this.state.objBank.cost
                            }}>
                            {({ handleSubmit, handleChange, handleBlur, values, touched, isValid, errors, }) => (
                                <Form noValidate onSubmit={handleSubmit} style={{ fontWeight: 'bold', fontSize: '25px' }}>
                                    <Form.Group controlId="name">
                                        <Form.Label>إسم البنك</Form.Label>
